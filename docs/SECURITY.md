...
- **One shared "access password"**, optional, set from Setup › Access. Once set, it
...
- The config (with credentials) only ever leaves the machine via an explicit **Export
  config** (Setup › Backup) that the operator triggers and downloads themselves —
...
- The control panel **binds to `0.0.0.0` by default** — every network interface on that
  ...
  unless `bind` is deliberately set to the AV VLAN's own address in Setup › System.
